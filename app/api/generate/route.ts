...
async function fetchOneFeed(url: string, sevenDaysAgo: number): Promise<{ title: string; link: string; date: string }[]> {
  try {
    ...
    for (const [, entry] of entries) {
      ...
      items.push({ title, link, date: date?.toISOString().split('T')[0] ?? '' })
...
async function fetchRSSNews(): Promise<{ items: { title: string; link: string; date: string }[]; elapsed: number }> {
  ...
  return { items, elapsed: Date.now() - start }
}
...
export async function POST() {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // RSS
      push({ type: 'rss_start', msg: `Fetching ${RSS_FEEDS.length} feed RSS in parallelo...` })
      const { items: rssItems, elapsed: rssMs } = await fetchRSSNews()
      const newsContext = rssItems.length > 0
        ? '\n\nNOTIZIE REALI DEGLI ULTIMI 7 GIORNI:\n' + rssItems.map(i => `- [${i.date}] ${i.title} | ${i.link}`).join('\n')
        : ''
      push({
        type: 'rss_done',
        msg: `${rssItems.length} notizie in ${(rssMs / 1000).toFixed(1)}s`,
        headlines: rssItems.slice(0, 8).map(i => `[${i.date}] ${i.title}`),
      })
